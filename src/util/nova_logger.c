...
void nova_log (const int level, const char * format, ...)
{
    ...
    {
        case NOVA_ERROR:
            p_level_str = "(Nova) ERROR: ";
            break;
        case NOVA_WARNING:
            p_level_str = "(Nova) WARNING: ";
            break;
        case NOVA_INFO:
        default:
            p_level_str = "(Nova) INFO: ";
            break;
    }
...
void nova_error (const char * format, ...)
{
    ...
    const char * err_str = strerror(saved_errno);
    snprintf(final_msg, sizeof(final_msg), "%s: %s", user_msg, err_str);

    nova_log(NOVA_ERROR, "%s", final_msg);
}
