 ...
 * @param p_resource_path The path of the resource relative to the current
 * running directory
 * @param p_resource_name The name of the resource to get
 * @return The path to the resource
 */
char * get_resource_location (const char * p_resource_path,
                              const char * p_resource_name)
{
    const char * p_resource_str = "/resources/";
    const char * p_running_path = get_current_running_path();
    const size_t size = strlen(p_running_path) + strlen(p_resource_str)
                        + strlen(p_resource_path) + strlen(p_resource_name) + 2;
    char *       p_full_resource_path = malloc(size);

    snprintf(p_full_resource_path,
             size,
             "%s%s%s%s%s",
             p_running_path,
             p_resource_str,
