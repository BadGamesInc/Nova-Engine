...
void calculate_zoom(Camera * p_camera)
{
    ...
    const float offset                  = p_camera->target_distance_to_entity - p_camera->distance_to_entity;
...
void calculate_angle_around_entity(Camera * p_camera)
{
    ...
    const float offset  = p_camera->target_angle_around_entity - p_camera->angle_around_entity;
    p_camera->angle_around_entity += offset * 0.01f * ROTATE_SMOOTHING;
}

void calculate_camera_pos(Camera * p_camera, const float h_distance, const float v_distance)
{
...
void update_view_matrix(Camera * p_camera)
{
    ...
    glm_rotate(p_camera->view_matrix, glm_rad(p_camera->pitch), (vec3) { 1.0f, 0.0f, 0.0f });
    glm_rotate(p_camera->view_matrix, glm_rad(p_camera->yaw), (vec3) { 0.0f, 1.0f, 0.0f });
    glm_rotate(p_camera->view_matrix, glm_rad(p_camera->roll), (vec3) { 0.0f, 0.0f, 1.0f });
    vec3 cam_pos = GLM_VEC3_ZERO_INIT;
    glm_vec3_copy(p_camera->position, cam_pos);
    vec3 neg_cam_pos = { -cam_pos[0], -cam_pos[1], -cam_pos[2] };
...
void camera_move (Camera * p_camera)
{
    ...
    float horizontal_distance = p_camera->distance_to_entity * cosf(glm_rad(p_camera->pitch));
    float vertical_distance   = p_camera->distance_to_entity * sinf(glm_rad(p_camera->pitch));
    calculate_camera_pos(p_camera, horizontal_distance, vertical_distance);
    p_camera->yaw = 180.0f - (p_camera->entity->rotation[1] + p_camera->angle_around_entity);
